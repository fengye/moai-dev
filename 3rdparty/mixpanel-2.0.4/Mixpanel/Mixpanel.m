...
- (NSString *)defaultDistinctId
{
    ...
    if (!distinctId && [[UIDevice currentDevice] respondsToSelector:@selector(identifierForVendor)]) {
        distinctId = [[UIDevice currentDevice].identifierForVendor UUIDString];
    }
    if (!distinctId && NSClassFromString(@"NSUUID")) {
        NSLog(@"%@ error getting device identifier: falling back to uuid", self);
        distinctId = [[NSUUID UUID] UUIDString];
    }
    if (!distinctId) {
        NSLog(@"%@ error getting uuid identifier: falling back to odin1", self);
        distinctId = ODIN1();
    }
    if (!distinctId) {
...
- (NSDictionary *)collectAutomaticProperties
{
    ...
    [p setValue:[self.mixpanel IFA] forKey:@"$ios_ifa"];
